...
namespace guild {
    ...
    public:
        ...
        auto dispatch(F &&f, Args &&... args)
            ...
            // 包装一个计算任务
            std::shared_ptr<std::packaged_task<R()> > task_ptr = std::make_shared<std::packaged_task<R()> >(
                std::packaged_task<R()>([f = std::forward<F>(f),...args=std::forward<Args>(args)]() mutable -> R {
                    return std::invoke(std::move(f), std::move(args)...);
                }));
            std::future<R> future = task_ptr->get_future();

            board_.post_quest([task_ptr] { (*task_ptr)(); });
            //是这样的，包装的任务是可调用对象，然后结果放在future里面，也就是只需要void()的调用即可生成结果。
