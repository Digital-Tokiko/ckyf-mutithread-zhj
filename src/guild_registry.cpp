...
namespace guild {
    ...
    std::optional<Attribute> Shard::get(const std::string &key) const {
        ...
        auto try_record = data_.find(key);
        if (try_record != data_.end() && !try_record->second.is_expired()) {
            return try_record->second.value;
        }
    ...
    void Shard::put(const std::string &key, Attribute value, std::chrono::seconds ttl) {
        ...
        auto target = data_.find(key); //应当记住insert特性。
        if (target == data_.end())
            data_.insert({
                key,
                Record{
            ...
            target->second.value = std::move(value);
            target->second.expire_at = (ttl.count() != 0)
                                           ? ttl + std::chrono::steady_clock::now()
                                           : std::chrono::steady_clock::time_point::max();
        }
    }
    ...
    Shard &GuildRegistry::get_shard(const std::string &key) {
        // TODO: 实现分片路由
        size_t shard_index = std::hash<std::string>{}(key) % shards_.size();

        return *shards_[shard_index]; // 删除此行，替换为正确实现
    }
    ...
    std::optional<Attribute> GuildRegistry::query(const std::string &key) {
        ...
        if (result.has_value()) stats_.hit_count.fetch_add(1);
    ...
    bool GuildRegistry::dismiss(const std::string &key) {
        // TODO: 实现删除，并更新统计计数器
        bool result = get_shard(key).remove(key);
